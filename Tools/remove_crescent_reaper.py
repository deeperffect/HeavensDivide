"""Remove the retired Crescent Reaper family and its exclusive artwork.

Run in Unreal's Python commandlet. Backups are kept under Saved/Backups.
Re-run with -ValidateCrescentRemoval to inspect saved assets without editing.
"""
import json
import shutil
...
ids={'CrescentReaper','CrescentReaperPower','CrescentReaperArea','CrescentReaperHaste',
     'ReapingReturn','TwinCrescents','CrimsonCrescent','ReapersInvitation'}
paths=['/Game/HeavensDivide/Upgrades/'+('Synergy/DA_BuildSynergy_' if uid=='ReapersInvitation' else 'Samurai/DA_Upgrade_Samurai')+uid for uid in sorted(ids)]
...
validate=unreal.SystemLibrary.get_command_line().find('-ValidateCrescentRemoval')>=0
backup=root/'Saved/Backups/CrescentReaperRemoval'/datetime.now().strftime('%Y%m%d_%H%M%S')
...
if not validate:
    ...
    assert len(pool)-len(retained) in (0,8), 'Unexpected removed upgrade count'
    ...
    unreal.log('CRESCENT_REMOVAL_BACKUP: '+str(backup))
...
for a in pool:
    assert str(a.get_editor_property('build_family_id'))!='CrescentReaper',a.get_path_name()
...
unreal.log('CRESCENT_REMOVAL_OK: 8 upgrades and 8 textures absent; no retired family or prerequisite in '+str(len(pool))+' pool entries')
