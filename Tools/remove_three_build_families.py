"""Remove the retired Fault Line, Thousand Cuts, and Spirit Lance families and its exclusive artwork.
...
ids={'FaultLine', 'SpectralBrand', 'CrossedLances', 'FaultDetonator', 'SpiritLance', 'OpenWounds', 'SpiritLancePower', 'SpiritLanceHaste', 'ExecutionWindow', 'RollingAftershock', 'SpiritLanceArea', 'SharedSentence', 'CrossFault', 'ThousandCuts', 'RuptureLift', 'FaultLinePower', 'ReboundingLance', 'ThousandCutsArea', 'FaultLineArea', 'ThousandCutsHaste', 'ThousandCutsPower', 'PassingSentence', 'ShadowSpear', 'FaultLineHaste'}
retired={'ThousandCuts', 'FaultLine', 'SpiritLance'}
synergies={'FaultDetonator', 'ShadowSpear', 'ExecutionWindow'}
paths=['/Game/HeavensDivide/Upgrades/'+('Synergy/DA_BuildSynergy_' if uid in synergies else 'Samurai/DA_Upgrade_Samurai')+uid for uid in sorted(ids)]
...
backup=root/'Saved/Backups/ThreeFamilyRemoval'/datetime.now().strftime('%Y%m%d_%H%M%S')
...
if not validate:
    ...
    assert len(pool)-len(retained) in (0,24), 'Unexpected removed upgrade count'
...
unreal.log('FAMILY_REMOVAL_OK: 24 upgrades and 24 textures absent; no retired family or prerequisite in '+str(len(pool))+' pool entries')
