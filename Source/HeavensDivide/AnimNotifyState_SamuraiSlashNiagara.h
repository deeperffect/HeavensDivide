...
#include "AnimNotifyState_SamuraiSlashNiagara.generated.h"

class UNiagaraSystem;
class UNiagaraComponent;

UCLASS()
class HEAVENSDIVIDE_API UAnimNotifyState_SamuraiSlashNiagara : public UAnimNotifyState
{
	GENERATED_BODY()

public:
	UAnimNotifyState_SamuraiSlashNiagara();
...
protected:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Niagara", meta = (ToolTip = "Immediately remove particles when the state ends. Otherwise stop emission and let existing particles finish."))
	bool bDestroyImmediately = false;

private:
	// Notify assets are shared by characters; keep each mesh's effect separate.
