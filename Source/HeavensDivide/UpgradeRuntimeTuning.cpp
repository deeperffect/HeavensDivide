...
{
 if(!Upgrades||Family<0||Family>=BuildFamilyCount||Slot==3)return nullptr;
 const auto& S=BuildFamilies[Family];const FName Id=Slot>=0&&Slot<3?S.Branches[Slot]:S.Id;
...
{
 if(Family<0||Family>=BuildFamilyCount)return Fallback;
...
{
 if(Family<0||Family>=BuildFamilyCount)return nullptr;
...
{
 if(Family<0||Family>=BuildFamilyCount)return Accent(Position,End,Radius,Color,Duration,bBeam);
