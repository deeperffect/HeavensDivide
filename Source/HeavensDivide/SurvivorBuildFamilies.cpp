...
{
 if(bResolvingReaction||Family<0||Family>=BuildFamilyCount||!BuildFamilies[Family].Available||!IsValid(Enemy)||Enemy->IsDead()||!Upgrades||Damage<=0) return;
...
void USurvivorAbilityComponent::GrantBuildPreview(FString FamilyId,int32 SelectedBranch)
{
 ...
 {
  const auto& S=BuildFamilies[i];if(!S.Available)continue;if(!FamilyId.Equals(S.Id,ESearchCase::IgnoreCase)&&!FamilyId.Equals(TEXT("All"),ESearchCase::IgnoreCase))continue;
