...
{ BuildCasts.Reset();BuildMarks.Reset(); }
void USurvivorAbilityComponent::UpdateBuildFamilies(ACharacterBase* Character)
{
 ...
 for(int32 i=5;i<BuildFamilyCount;++i)
 {
...
{
 if(Family<5||Family>=BuildFamilyCount||!IsValid(Character)||!Upgrades||!Upgrades->HasUpgradeId(BuildFamilies[Family].Id)||BuildCasts.Num()>80) return false;
 ...
 case EBuildPattern::Zone:
  {
   ...
   BuildCircle(C,C.Origin,R,1,D,true);
   if(B) BuildCircle(C,C.End,R*Tuning(C.Family,TEXT("SecondaryRadiusMultiplier"),1,1),Tuning(C.Family,TEXT("SecondaryDamageMultiplier"),1,1),D,true);break;
  }
 case EBuildPattern::Swarm:
  {
   ...
   BuildDamage(C,C.Target.Get(),1,true);
   if(B) for(auto* E:FindEnemies(C.End,R,Source)) if(E!=C.Target.Get()) BuildDamage(C,E,Tuning(C.Family,TEXT("SplashDamageMultiplier"),0.5f,1),true);
   if(D&&C.Step==C.Steps-1) BuildCircle(C,C.End,R*Tuning(C.Family,TEXT("FinalRadiusMultiplier"),2.0f,2),Tuning(C.Family,TEXT("FinalDamageMultiplier"),3.0f,2),true);break;
  }
...
{
 if(bResolvingReaction||Family<0||Family>=BuildFamilyCount||!IsValid(Enemy)||Enemy->IsDead()||!Upgrades||Damage<=0) return;
 const auto Source=FamilySource(Family);
 const float Duration=FMath::Max(0.1f,PreparationDuration)+Upgrades->GetMetaSkillBonus(TEXT("Preparation"));
 for(auto& Mark:BuildMarks) if(Mark.Enemy==Enemy)
 {
  // An opposite-source lingering ability must not steal the existing preparation.
  if(Mark.Source==Source||Mark.Remaining<=0){Mark.Source=Source;Mark.Damage=Damage;Mark.Remaining=Duration;}
  return;
 }
 if(BuildMarks.Num()<512){FBuildMark Mark;Mark.Enemy=Enemy;Mark.Source=Source;Mark.Damage=Damage;Mark.Remaining=Duration;BuildMarks.Add(Mark);}
}
bool USurvivorAbilityComponent::HasTriggerablePreparation(EPlayerAttackSource Source,const AEnemyBase* Enemy) const
{
 if(!Upgrades||!IsValid(Enemy)||Enemy->IsDead()||!Enemy->CanReceivePlayerDamage(Source)) return false;
 if(Source!=EPlayerAttackSource::Samurai&&Source!=EPlayerAttackSource::Ninja) return false;
...
void USurvivorAbilityComponent::PrioritizePreparedTargets(EPlayerAttackSource Source,TArray<AEnemyBase*>& Targets) const
{
 TSet<const AEnemyBase*> Prepared;
 for(const auto* Enemy:Targets) if(HasTriggerablePreparation(Source,Enemy)) Prepared.Add(Enemy);
...
void USurvivorAbilityComponent::NotifyPartnerHit(EPlayerAttackSource Source,AEnemyBase* Enemy,bool bAssistHit,uint8 StatusBeforeHit)
{
 if(!bAssistHit||bResolvingReaction||!Controller||!Upgrades||!IsValid(Enemy)||!Controller->IsRunInProgress()||Controller->IsPlayerDead()) return;
 if(Source!=EPlayerAttackSource::Samurai&&Source!=EPlayerAttackSource::Ninja) return;
 if(!Enemy->IsDead()&&!Enemy->CanReceivePlayerDamage(Source)) return;
 ...
 const bool bBleed=StatusBeforeHit==255?Enemy->HasStatus(EEnemyStatusEffect::Bleed):(StatusBeforeHit&1)!=0;
 const bool bPoison=StatusBeforeHit==255?Enemy->HasStatus(EEnemyStatusEffect::Poison):(StatusBeforeHit&2)!=0;
 Enemy->ApplyPlayerDamage(Prepared.Damage*FMath::Max(0.0f,PreparationDamageMultiplier)*(1.f+Upgrades->GetMetaSkillBonus(TEXT("Reaction"))),Source);
 ...
 {
  if(!(Status==EEnemyStatusEffect::Bleed?bBleed:bPoison)||Radius<=0) continue;
  const auto StatusSource=Status==EEnemyStatusEffect::Bleed?EPlayerAttackSource::Samurai:EPlayerAttackSource::Ninja;
  for(auto* Target:FindEnemies(Position,Radius,StatusSource))
   if(Target!=Enemy) Target->GetStatusEffectComponent()->ApplyStatus(Status,Upgrades,StatusSource,true);
 }
}
...
void USurvivorAbilityComponent::GrantBuildPreview(FString FamilyId,int32 SelectedBranch)
{
 ...
 for(int32 i=0;i<BuildFamilyCount;++i)
 {
  ...
  Controller->ClientMessage(FString::Printf(TEXT("%s build ready: scaling rank 2, selected branches. Universal Prepare is always available. This run only."),S.Title));
 }
#endif
}
